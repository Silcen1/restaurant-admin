<template>
    <div class="xfn-category-list">
        <el-breadcrumb>
            ...
            <el-breadcrumb-item to="/main">菜品类别管理
        ...
        <el-table :data="categoryList" stripe border>
            ...
            <el-table-column label="操作">
                <template slot-scope="{row,$index}">
                  ...
                  <el-button @click="deleteCategory(row,$index)" type="error" size="mini">删除
                      </el-button>
                </template>
            </el-table-column>
        </el-table>
    ...
    export default {
        ...
        methods:{
           deleteCategory(c,i){
              ...
              var url=this.$store.state.globalSetting.apiUrl+'/admin/category/'+c.cid;
              this.$axios.delete(url).then((res)=>{
                  if(res.data.code==200){ //数据库中已经删除成功
                  this.categoryList.splice(i,1); //模型数据中删除
                  this.$message.success('菜品类别删除成功! ');
                  }else{
                      this.$message.error('菜品类别删除出错! '+res.data.msg);
                  }
              }).catch((err)=>{
                  console.log(err);
              })
           ...
           updateCategory(c,i){
               this.$prompt('请输入修改的菜品类别名: ','提示',{inputValue:c.cid}).then(({value})=>{
                   //获取用户的输入，调用数据API添加到数据库
                   var url=this.$store.state.globalSetting.apiUrl+'/admin/category';
                   this.$axios.put(url,{cid:c.cid,cname:value}).then((res)=>{
                       if(res.data.code==200){
                           this.$message.success('修改类别添加成功! ');
                           //模型数据中添加新的类别
                           this.categoryList[i].cname=value;
                       }else{
                           this.$message.error('修改类别添加出错:'+res.data.msg);
                       }
                   }).catch((err)=>{
                       console.log(err);
                   })
